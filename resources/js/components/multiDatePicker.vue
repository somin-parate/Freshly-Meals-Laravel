    ...
    <div class="">
        ...
        <transition name="smooth">
            ...
            >
                <div class="dp-header">
                    <div
                        class="btn btn-link last-year"
                        @click="seleYear--"
                    ></div>
                    <div
                        class="btn btn-link last-month"
                        @click="changeMonth(-1)"
                    ></div>
                    <div class="btn btn-link sele-year">
                        {{ seleYear }}
                        <span v-if="lang != 'en' || !!disp">{{
                            display.year
                        }}</span>
                    </div>
                    <div class="btn btn-link sele-month">
                        <span v-if="lang === 'en' && !disp">{{
                            monthNames[seleMonth]
                        }}</span>
                        <span v-else
                            >{{ seleMonth + 1 }}{{ display.month }}</span
                        >
                    </div>
                    <div
                        class="btn btn-link next-month"
                        @click="changeMonth(1)"
                    ></div>
                    <div
                        class="btn btn-link next-year"
                        @click="seleYear++"
                    ></div>
                </div>
                <div class="dp-body">
                ...
                <div class="dp-footer" v-show="multi">
                    <div class="btn btn-cancel" @click="cancelSelect">
